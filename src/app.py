...
db_engine.execute("""INSERT INTO publishers(publisher_id, name) VALUES (1, 'O Reilly Media');
...
print(store_db_engine_authors)

#Opción B)
df_authors_2 = db_engine.execute("SELECT * FROM book_authors") #SEE HOW THIS WORKS BELOW
for row in df_engine_authors: # Iterate through all the rows of the resulting filter
    print(f"book_id = {row[0]}  author_id = {row[1]}")
